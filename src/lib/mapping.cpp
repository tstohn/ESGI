...
void Mapping<MappingPolicy>::initialize_mapping()
{

}

template <typename MappingPolicy>
bool Mapping<MappingPolicy>::run_mapping(const std::string& seq, input* input, BarcodeMapping& barcodeMap, BarcodeMapping& realBarcodeMap,
...
bool check_if_seq_too_short(const int& offset, const std::string& seq, const input& input, const int& len)
{
                                      ...
                                      BarcodePatternVectorPtr barcodePatterns)
{
        ...
        ++patternItr)
    {
        ...
        bool seqToShort = check_if_seq_too_short(offset, seq, *input, (*patternItr)->get_patterns().at(0).length());
        ...
        std::cout << "#matching pttrn " << seq << " " << offset << " " << start << " "<< (*patternItr)->mismatches << "\n";
        if(!(*patternItr)->match_pattern(seq, offset, start, end, score, realBarcode, differenceInBarcodeLength, stats, startCorrection))
        {
            ...
            {
                std::cout << "got a wrong pattern\n";
                realBarcode = "";
                start = 0;
                end = 0;
            }
        }
        std::cout << " diffrerence calcualtion after mapping|" << realBarcode << "|\n";
...
void split_barcodes_in_subset(input& input, BarcodeMappingVector& barcodes, BarcodeMappingVector& realBarcodes, BarcodePatternVectorPtr barcodePatterns, fastqStats& fastqStatsFinal,
                              const std::vector<std::pair<std::string, char> >& patterns, std::vector<std::string>& fastqLines, fastqStats& emptyStats)
{
...
template class Mapping<MapEachBarcodeSequentiallyPolicy>;
template class Mapping<MapEachBarcodeSequenciallyWithLeaveOutsPolicy>;
template class Mapping<MapAroundConstantBarcodesAsAnchorPolicy>;