...
class UnprocessedDemultiplexedData
{
    public:
        ...
        void add_to_umiDict(const char* umiChar, std::string& abStr, std::string& singleCellStr, std::vector<std::string>& annotations)
        {
            ...
            for(std::string& annotation :annotations)
            {
                line.annotations.push_back(uniqueChars->getUniqueChar(annotation.c_str()));
            }
        ...
        // add a dataLines to the vector
        void add_to_scAbDict(const char* umiChar, std::string& abStr, std::string& singleCellStr, std::vector<std::string>& annotations)
        {
            //get unique pointer for all three strings
            dataLine line;
            line.umiSeq = uniqueChars->getUniqueChar(umiChar);
        ...
        inline std::string getAnnotationName(const int annotationIdx, const std::string& barcode) const
        {
            if ( annotationDict.find(annotationIdx) == annotationDict.end() ) 
            {
                std::cerr << "The column of annotation index seems to not exist:\n" << annotationIdx << "\n";
                exit(EXIT_FAILURE);
            }
            else if ( annotationDict.at(annotationIdx).find(barcode) == annotationDict.at(annotationIdx).end() ) 
            {
                std::cerr << "Barcode is not in treatment dict, check treatment barcodes, treatmend ID and treatment File:\n" << barcode << "\n";
                exit(EXIT_FAILURE);
            }
            return (annotationDict.at(annotationIdx)).at(barcode);
        }
    ...
    private:
        ...
        //dictionaries to map an annotation index to (annotation barcode => annotation string like treatment, spatial coords, ...)
        //those dicts are used in the very beginning when lines r parsed, so the real barcode sequence is never stored
        std::unordered_map< int, std::unordered_map<std::string, std::string >> annotationDict;
